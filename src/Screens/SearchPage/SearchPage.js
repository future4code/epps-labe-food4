...
function SearchPage() {
    ...
    const render = filteredList && filteredList.map((res) => {
    ...
    const contentSearch = <ContainerTitleSearch><p>Não encontramos :(</p></ContainerTitleSearch>
    return (
        <div>
            ...
            <ContainerCardFeed>{inputName !== '' ? 
            <>{render.length > 0 ? render : contentSearch}</> : 
            <ContainerTitleSearch>
                <p>{inputName == '' ? 'Busque por nome de restaurante' : <>{filteredList.length == 0 && <p>Não encontramos</p>}</>}</p>
            </ContainerTitleSearch>}</ContainerCardFeed>
        </div>
    )
}
